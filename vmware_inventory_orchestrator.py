...
def batch_fetch_properties(content, obj_type, properties):
    ...
    property_spec = vmodl.query.PropertyCollector.PropertySpec(
        type=obj_type, pathSet=properties
    )

    traversal_spec = vmodl.query.PropertyCollector.TraversalSpec(
        name="traverseEntities", path="view", skip=False, type=container_view.__class__
    )
    ...
    results = {
        obj.obj._moId: {prop.name: prop.val for prop in obj.propSet} for obj in props
    }
    return results
...
def process_custom_attributes(virtual_machines, content):
    ...
    for vm_id, vm_properties in virtual_machines.items():
        ...
        processed_custom_values = {
            all_custom_attributes.get(cv.key, cv.key): cv.value for cv in custom_values
        }
...
def process_vm_info(vm_properties):
    """Processes and returns a dictionary of key VM information, including disk provisioning types."""
    harddisk_info = []
    for device in vm_properties.get("config.hardware.device", []):
        if isinstance(device, vim.vm.device.VirtualDisk):
            # Initialize disk_type as 'unknown' by default
            disk_type = 'unknown'

            # Check for the backing type and set disk_type accordingly
            if hasattr(device.backing, 'thinProvisioned'):
                if device.backing.thinProvisioned:
                    disk_type = 'thin'
                elif hasattr(device.backing, 'eagerlyScrub') and device.backing.eagerlyScrub:
            ...
            for ctrl_device in vm_properties.get("config.hardware.device", []):
                if ctrl_device.key == device.controllerKey:
                    controller_type = type(ctrl_device).__name__.replace('Virtual', '')
                    controller_number = ctrl_device.busNumber
                    break

            disk_info = {
                "label": device.deviceInfo.label,
                "capacity_gb": round(device.capacityInKB / (1024**2), 2),
                "datastore": device.backing.datastore.summary.name,
                "type": disk_type,
                "controller_type": controller_type,
                "controller_number": controller_number,
                "unit_number": device.unitNumber
            }
            harddisk_info.append(disk_info)

    power_state = vm_properties.get("runtime.powerState", "unknown")
    ...
    for nic in vm_properties.get("guest.net", []):
        if nic.ipConfig:
            ...
            ip_info[interface_name] = [
                f"{ip.ipAddress}/{ip.prefixLength}" for ip in nic.ipConfig.ipAddress
            ]
    return ip_info
...
def merge_vminfo_with_rack_data(vminfo, rack_data):
    ...
    for vm in vminfo:
        if not append_vm_to_esxi(vm, rack_data):
            logging.info(
                f"No matching ESXi found for VM '{vm['name']}' on host '{vm['host']}'"
            )
    return rack_data
...
def main():
    ...
    if connection:
        ...
        vminfo = [
            process_vm_info(vm_properties)
            for vm_properties in vm_inventory_data.values()
        ]
        ...
        vms_with_empty_custom_attributes = [
            vm for vm in vminfo if not vm["custom_attributes"]
        ]
